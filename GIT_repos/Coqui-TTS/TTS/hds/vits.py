...
class VitsHDS(Vits):
    ...
    def train_step(self, batch: dict, criterion: List[str], optimizer_idx: int) -> Tuple[Dict, Dict]:
        ...
        # Generator
            ...
            loss_feat = (
                    self.feature_loss(feats_real=scores_disc_real,
                                      feats_generated=scores_disc_fake) * self.config.feat_loss_alpha
            )
    ...
    def duration_loss(self, dur, x, x_mask):
        ...
        loss = torch.sum((durations_pred - durations) ** 2) / torch.sum(x_mask)
        loss = torch.sum(loss)
